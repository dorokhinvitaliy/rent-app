...
export async function exportWorkbook(listings: Listing[], months: number) {
  ...
  listings.forEach((l, i) => {
    ...
    sheet.addRow([
      ...
      { formula: `E${r}+F${r}`, result: c.monthly },
      { formula: `L${r}+G${r}+K${r}+J${r}`, result: c.moveIn },
      months,
      { formula: `L${r}*N${r}+K${r}+J${r}`, result: c.total },
      { formula: `O${r}+G${r}`, result: c.cashTotal },
      { formula: `O${r}/N${r}`, result: c.average },
      c.incomplete ? 'Неполные: уточните суммы' : 'Все расходы указаны',
      l.notes,
      l.photos[0] || '',
    ...
    [5, 6, 7, 10, 11, 12, 13, 15, 16, 17].forEach(
      (col) => (sheet.getCell(r, col).numFmt = '#,##0.00'),
    );
    ...
    if (i % 2 === 0)
      sheet
        ...
        .eachCell(
          (cell) =>
            (cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF2F5F1' } }),
        );
  });
  sheet.getRow(1).height = 36;
  sheet.getRow(1).eachCell((c) => {
    c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF26473C' } };
  ...
  guide.addRows([
    ...
    [
      'Расходы за срок',
      'Аренда и ЖКУ × число месяцев + комиссия + прочие расходы. Возвратный залог исключен.',
    ],
    ['С учетом залога', 'Все денежные выплаты за срок до возврата залога.'],
    [
      'Неизвестные расходы',
      'Пустая ячейка означает «не указано», а не ноль. Формулы считают нижнюю оценку; уточните суммы.',
    ],
    [
      'ЖКУ',
      'Указывайте свою оценку, включая счетчики. Неизвестные суммы не подставляются автоматически.',
    ],
    [
      'Фотографии',
      'Ссылки на первое фото. Изображения не встроены в файл и могут быть недоступны у источника.',
    ],
